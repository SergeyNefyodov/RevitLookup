...
public static class RibbonExtensions
{
    ...
    public static RibbonButton SetImage(this RibbonButton button, string uri)
    {
        ...
        var themedIconUri = GetThemedIconUri(uri);
        if (IsThemedIconUri(themedIconUri))
        {
            MonitorThemedButton(button);
        }
#else
    ...
    /// <summary>
    ///     Sets the tooltip text for the PushButton.
    /// </summary>
    /// <param name="button">The PushButton to which the tooltip will be added.</param>
    /// <param name="tooltip">The text to display as a tooltip when the mouse pointer hovers over the button.</param>
    /// <returns>The PushButton with the specified tooltip text.</returns>
    /// <remarks>
    ...
    private static void MonitorThemedButton(RibbonButton button)
    {
        Context.UiApplication.ThemeChanged -= OnButtonThemeChanged;
        Context.UiApplication.ThemeChanged += OnButtonThemeChanged;
        return;

        void OnButtonThemeChanged(object? sender, ThemeChangedEventArgs args)
        {
            if (args.ThemeChangedType != ThemeType.UITheme) return;

            if (button.Image is BitmapImage image)
            {
                var themedIconUri = GetThemedIconUri(image.UriSource.OriginalString);
                button.Image = new BitmapImage(new Uri(themedIconUri, UriKind.RelativeOrAbsolute));
            }

            if (button.LargeImage is BitmapImage largeImage)
            {
                var themedIconUri = GetThemedIconUri(largeImage.UriSource.OriginalString);
                button.LargeImage = new BitmapImage(new Uri(themedIconUri, UriKind.RelativeOrAbsolute));
            }
        }
    }
    ...
    /// <summary>
    ///     Determines if the given URI corresponds to a themed icon (e.g., contains "Light" or "Dark").
    /// </summary>
    /// <param name="uri">The URI to check.</param>
    /// <returns><c>true</c> if the URI is themed; otherwise, <c>false</c>.</returns>
    public static bool IsThemedIconUri(string uri)
    {
        return uri.Contains("Light") || uri.Contains("Dark");
    }

    /// <summary>
    ///     Modifies the given URI to match the current UI theme.
    /// </summary>
    /// <param name="uri">The original URI.</param>
    /// <returns>The modified URI corresponding to the current UI theme.</returns>
    private static string GetThemedIconUri(string uri)
    {
        return UIThemeManager.CurrentTheme switch
        {
            UITheme.Light when uri.Contains("Dark") => uri.Replace("Dark", "Light"),
            UITheme.Light => uri,
            UITheme.Dark when uri.Contains("Light") => uri.Replace("Light", "Dark"),
            UITheme.Dark => uri,
